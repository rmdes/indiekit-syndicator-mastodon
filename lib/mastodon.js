...
import { createStatus, createLikeStatus, createRepostStatus, getStatusIdFromUrl } from "./utils.js";
...
export class Mastodon {
  ...
  /**
   * Resolve a remote status URL to a local status ID
   * Uses Mastodon v2/search with resolve:true to federate and cache the remote status
   * @param {string} statusUrl - URL of a status on a remote instance
   * @returns {Promise<string|null>} Local status ID or null if not found
   */
  async resolveRemoteStatus(statusUrl) {
    try {
      const { v2 } = this.#client();
      const results = await v2.search.fetch({
        q: statusUrl,
        type: "statuses",
        resolve: true,
        limit: 1,
  ...
  /**
   * Get the local status ID for a given status URL
   * For same-instance URLs, extracts ID from the URL path.
   * For remote URLs, resolves via search API.
   * @param {string} statusUrl - URL of the status
   * @returns {Promise<string>} Local status ID
   */
  async #getLocalStatusId(statusUrl) {
    ...
    const resolvedId = await this.resolveRemoteStatus(statusUrl);
    if (!resolvedId) {
      throw new Error(
        `Could not resolve remote status: ${statusUrl}`,
      );
    }
    return resolvedId;
  }
  ...
  async post(properties, me) {
    ...
    // Cross-instance reply resolution: if in-reply-to exists but inReplyToId
    // wasn't set (different hostname), try to resolve via search API
    if (
      properties["in-reply-to"] &&
      !status.inReplyToId &&
      !isSameOrigin(properties["in-reply-to"], this.serverUrl)
    ) {
      const resolvedId = await this.resolveRemoteStatus(
        properties["in-reply-to"],
      );
      if (resolvedId) {
        status.inReplyToId = resolvedId;
      }
    }
